/*
  # Add User IDs to Analytics Function

  1. Updates
    - Modifies get_analytics_data() to include user_id in highest_user_balance
    - Adds user_id to most_recent_deposit
    - Adds user_id to most_recent_payout
    - Enables linking to user profiles from analytics cards

  2. Return Structure Changes
    - highest_user_balance: { amount, user_name, user_id }
    - most_recent_deposit: { amount, date, user_name, user_id }
    - most_recent_payout: { amount, date, user_name, user_id }
*/
...
BEGIN
  ...
  -- Highest User Balance with user name and user_id
  SELECT jsonb_build_object(
  ...
  highest_user_balance := COALESCE(v_highest_user_balance, jsonb_build_object('amount', 0, 'user_name', '', 'user_id', ''));
  
  -- Most Recent Deposit with user_id
  SELECT jsonb_build_object(
    'amount', t.amount,
    'date', t.created_at,
    'user_name', p.first_name || ' ' || p.last_name,
  ...
  -- Most Recent Payout with user_id
  SELECT jsonb_build_object(
    'amount', t.amount,
    'date', t.created_at,
    'user_name', p.first_name || ' ' || p.last_name,

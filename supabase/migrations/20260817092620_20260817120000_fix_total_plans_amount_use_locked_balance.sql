-- Change total_plans_amount to sum locked_balance for users with active payout plans
-- (previously summed total_amount from payout_plans, which was the wrong metric)
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COALESCE(SUM(w.locked_balance), 0)
       FROM wallets w
       WHERE w.user_id IN (
         SELECT DISTINCT user_id FROM payout_plans WHERE status = 'active'
       )

/*
  # Add Comprehensive Analytics Metrics to get_analytics_data()

  1. Updates
    - Drops and recreates get_analytics_data() function with 7 new metrics
    - Adds total_users: Total count of registered users
    - Adds total_users_balance: Sum of all wallet balances (balance + locked_balance)
    - Adds total_amount_in_plans: Sum of total_amount in active/paused payout plans
    - Adds total_completed_payouts: Count of completed payout plans
    - Adds highest_user_balance: Maximum wallet balance (balance + locked_balance)
    - Adds most_recent_deposit: Most recent deposit transaction with user details
    - Adds most_recent_payout: Most recent payout transaction with user details

  2. Return Structure
    - All existing fields remain unchanged
    - New fields added to function return type
*/
...
DECLARE
  ...
  v_highest_user_balance numeric;
...
BEGIN
  ...
  SELECT COALESCE(MAX(balance + locked_balance), 0) INTO v_highest_user_balance FROM wallets;
  highest_user_balance := v_highest_user_balance;

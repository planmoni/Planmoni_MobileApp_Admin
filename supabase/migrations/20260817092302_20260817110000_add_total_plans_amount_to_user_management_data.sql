-- Add total_plans_amount to get_user_management_data
-- Must DROP first since the return type changed (new column added)
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COALESCE(SUM(total_amount), 0) FROM payout_plans WHERE status = 'active') as total_plans_amount,

/*
  # Complete Super Admin Role and Permission System
  ...
  2. Functions
    - is_super_admin(): Check if user has super admin privileges
    - has_permission(): Check if user has specific permission
    - assign_user_role(): Assign role to user with validation
    - revoke_user_role(): Remove role from user
    - get_user_roles(): Get all roles for a user
    - get_user_permissions(): Get all permissions for a user
    - get_all_roles_with_permissions(): Get roles with their permissions
    - get_super_admin_stats(): Get system statistics

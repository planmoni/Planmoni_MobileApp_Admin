...
BEGIN
  ...
  RETURN QUERY EXECUTE '
    SELECT
      ...
      t.created_at,
      t.metadata,
      pa.bank_name as destination_bank_name,
      pa.account_number as destination_account_number,
      pa.account_name as destination_account_name,
      ' || total_records || '::bigint as total_count
    FROM transactions t
    LEFT JOIN profiles p ON t.user_id = p.id
    LEFT JOIN payout_plans pp ON t.payout_plan_id = pp.id
    LEFT JOIN bank_accounts ba ON t.bank_account_id = ba.id
    LEFT JOIN automated_payouts ap ON (t.metadata->>''automated_payout_id'')::uuid = ap.id
    LEFT JOIN payout_accounts pa ON ap.payout_account_id = pa.id
    ' || where_clause || '

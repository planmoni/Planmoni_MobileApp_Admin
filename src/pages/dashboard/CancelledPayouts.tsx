...
export default function CancelledPayouts() {
  ...
  const cancelledPlans = payoutPlans.filter((p: any) => p.status === 'cancelled');
    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-1">Cancelled Payouts</h1>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-soft border border-gray-100 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500">Total Cancelled Payouts</p>
            <p className="text-2xl font-bold text-gray-900">{cancelledPlans.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-soft border border-gray-100 overflow-hidden">
        {cancelledPlans.length > 0 ? (
              ...
              return (
                <div key={plan.id} className="p-6 hover:bg-gray-50 transition-colors">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="px-3 py-1 rounded-lg text-xs font-semibold bg-red-50 text-red-600 border border-red-100">
                      Cancelled
                    </span>
                  </div>
                  ...
                  {endDate && !isNaN(endDate.getTime()) && (
                    <div className="mb-3 p-3 bg-gray-50 rounded-lg">
                      <p className="text-xs font-medium text-gray-500 mb-1">Would Have Ended On</p>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs text-gray-500">
                      <span>{Math.round((plan.completed_payouts / plan.duration) * 100)}% completed before cancellation</span>
                      <span>{plan.duration - plan.completed_payouts} payouts cancelled</span>
                    </div>
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-gray-500 font-medium">No cancelled payouts found</p>
            <p className="text-gray-400 text-sm mt-1">User hasn't cancelled any payout plans</p>
          </div>
        )}
      </div>
    </div>

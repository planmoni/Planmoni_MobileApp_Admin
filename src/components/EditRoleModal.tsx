...
interface EditRoleModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (roleId: string, roleData: RoleFormData) => void;
  role: Role | null;
...
export function EditRoleModal({ isOpen, onClose, onSubmit, role, permissions }: EditRoleModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (role) {
      onSubmit(role.role_id, formData);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            {role.is_system && (
              <p className="text-sm text-blue-600 mt-1">System Role - Some fields are restricted</p>
            )}
          </div>
          <button
            onClick={onClose}
